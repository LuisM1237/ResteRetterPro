...
st.markdown(f"""<style>
    ...
    .block-container {{background: rgba(255,255,255,0.95); padding: 2rem!important; border-radius: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.4); margin-top: 20px;}}
    h1 {{color: #1e8449!important; text-align: center; font-family: 'Arial Black';}}
    
    /* Expander-Titel Fix für Handy-Lesbarkeit */
    .streamlit-expanderHeader {{
        font-size: 1.1rem !important;
        color: #1a1a1a !important; /* Dunkles Anthrazit für Lesbarkeit */
        font-weight: bold !important;
        background-color: rgba(255, 255, 255, 0.8) !important;
        border-radius: 10px;
    }}
    
    .m {{color: #e74c3c; font-weight: bold; font-size: 1rem;}}
    .s {{color: #27ae60; font-weight: bold; font-size: 1rem;}}
</style>""", unsafe_allow_html=True)

if 'init' not in st.session_state:
    st.markdown("<style>.block-container {visibility: hidden;}</style><h1 style='color: white; margin-top: 200px; text-shadow: 2px 2px 8px #000;'>🥗 ResteRetter Pro wird geladen...</h1>", unsafe_allow_html=True)
    time.sleep(2); st.session_state.init = True; st.rerun()

# --- UI ---
st.title("🥗 ResteRetter Pro")
st.markdown("<p style='text-align: center; color: #333;'>Gib deine Zutaten ein und wir finden das perfekte Essen!</p>", unsafe_allow_html=True)
st.caption("_Format: Zutat, Zutat, ..._")
u_in = st.text_input("Was hast du noch im Kühlschrank?", placeholder="z.B. Tomaten, Käse, Eier")
...
if st.button("JETZT REZEPTE FINDEN") and u_in:
    with st.spinner('⚡ Suche läuft...'):
        # Korrektur: Split an jedem Komma
        ...
        for ing in st.session_state.u_en:
            ...
            if res_api and 'meals' in res_api and res_api['meals']:
                for m in res_api['meals']: ids.add(m['idMeal'])
        ...
        for m_id in list(ids)[:12]:
            ...
            match_count = sum(1 for user_ing in st.session_state.u_en if any(user_ing in recipe_ing for recipe_ing in r_i))
...
if st.session_state.res is not None:
    if not st.session_state.res:
        st.markdown("---")
        try: st.image(Image.open("Hungerbild.jpg"), use_container_width=True)
        except: st.warning("Keine Rezepte gefunden.")
    else:
        for i, item in enumerate(st.session_state.res):
            d = item['d']
            # Der Header wird nun durch das CSS oben direkt lesbar gemacht
            header = f"{d['strMeal'].upper()} | ✅ {item['m']} | 🛒 {item['miss']}"
